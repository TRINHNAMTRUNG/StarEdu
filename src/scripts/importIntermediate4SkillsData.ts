...
/**
 * Script import INTERMEDIATE 4 Skills (L&R 550+ & S&W 200+) data
 */
...
async function importIntermediate4SkillsData() {
  try {
    ...
    console.log("\n🗑️  Deleting old Intermediate 4 Skills roadmap data...");
    const oldIntermediate4SkillsRoadmap = await RoadmapModel.findOne({ title: /Trung Cấp 4 Kỹ Năng.*550.*200/ });
    if (oldIntermediate4SkillsRoadmap) {
      const oldCourseIds = oldIntermediate4SkillsRoadmap.courses;
      ...
      await RoadmapModel.deleteOne({ _id: oldIntermediate4SkillsRoadmap._id });
      console.log("✅ Old intermediate 4 skills data deleted");
    } else {
      console.log("✅ No old intermediate 4 skills data to delete");
    }

    console.log("\n" + "=".repeat(70));
    console.log("🗺️  ROADMAP: Lộ Trình Trung Cấp 4 Kỹ Năng - 550+ & 200+");
    ...
    const course1 = await CourseModel.create({
      ...
      description: "Khóa học nâng cao kỹ năng nghe Part 1-2 với câu hỏi thật từ ETS. Phù hợp cho người đã có nền tảng cơ bản.",
    ...
    const roadmap = await RoadmapModel.create({
      title: "Lộ Trình Trung Cấp 4 Kỹ Năng - 550+ & 200+",
      description: "Lộ trình học TOEIC hoàn chỉnh 4 kỹ năng trung cấp với câu hỏi thật từ ETS 2020. Gồm 6 khóa học: Listening & Reading (550+) + Speaking & Writing (200+).",
      skill_groups: ["listening", "reading", "speaking", "writing"],
      target_score: 750, // 550 + 200
    ...
    console.log(`   - Target: L&R 550+ & S&W 200+`);

    console.log("\n" + "=".repeat(70));
    console.log("✅ Import hoàn tất - Lộ Trình Trung Cấp 4 Kỹ Năng!");
    console.log("=".repeat(70));
